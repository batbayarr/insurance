-- PostgreSQL Subsidiary Ledger Function
-- This function returns transaction details for a specific account and client within a date range
-- Parameters: AccountId INTEGER, ClientId INTEGER (nullable), beginDate DATE, EndDate DATE
...
DROP FUNCTION IF EXISTS public.report_subsidiary_ledger(INTEGER, INTEGER, DATE, DATE);

CREATE OR REPLACE FUNCTION public.report_subsidiary_ledger(
    AccountId INTEGER,
    ClientId INTEGER,
    beginDate DATE,
    EndDate DATE
...
BEGIN
    ...
    WHERE cdd."AccountId" = AccountId
        AND (ClientId IS NULL OR cdd."ClientId" = ClientId)
    ...
    WHERE idd."AccountId" = AccountId
        AND (ClientId IS NULL OR idd."ClientId" = ClientId)
    ...
    WHERE add."AccountId" = AccountId
        AND (ClientId IS NULL OR add."ClientId" = ClientId)
...
-- Example usage:
-- SELECT * FROM report_subsidiary_ledger(1, 2, '2025-01-01', '2025-12-31');
-- SELECT * FROM report_subsidiary_ledger(1, NULL, '2025-01-01', '2025-12-31'); -- All clients for account 1
