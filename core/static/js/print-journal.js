/**
 * Generic Journal Print System
 * 
 * Handles landscape A4 printing for journal tables (Cash, Inventory, Asset)
 * All journals use the same format with identical columns
 * Excludes "AccountName" and "User" columns
 * 
 * Usage:
 * window.printJournalTable({
 *     title: 'Cash Journal - ЖУРНАЛ',
 *     startDate: '2025-01-01',
 *     endDate: '2025-01-31',
 *     allData: detailsData,
 *     columnHeaders: ['Баримт', 'Огноо', 'Данс', ...],
...
(function() {
    ...
    /**
     * Map data field names to column headers
     */
    function getDataFieldMapping() {
        return {
            ...
            'Огноо': 'document_date',
            'Данс': 'account_code',
            'Харилцагч': 'client_name',
            'Гүйлгээний утга': 'description',
            'Валют': 'currency_name',
            'Валютын дүн': 'currency_amount',
            'Ханш': 'currency_exchange',
            'Дебит дүн': 'debit_amount',
            'Кредит дүн': 'credit_amount'
        };
    }
    ...
    function generateTableRows(data, columnHeaders, fieldMapping) {
        ...
        data.forEach((item, index) => {
            ...
            columnHeaders.forEach(header => {
                ...
                let isNumeric = false;
                if (fieldName === 'currency_amount' || 
                    fieldName === 'currency_exchange' || 
                    fieldName === 'debit_amount' || 
                    fieldName === 'credit_amount') {
                    value = formatNumber(value, fieldName === 'currency_exchange' ? 4 : 2);
                    isNumeric = true;
...
<body style="margin: 0 !important; padding: 0 !important; background: #fff !important; width: 100% !important; height: 100% !important;">
    ...
    <script type="text/javascript">
        ...
        function exportToExcel() {
            ...
            alert('Journal exported to Excel successfully! The file should open automatically if Excel is your default application for CSV files.');
        }
     ...
     * Main print function
     */
    window.printJournalTable = function(config) {
        // Validate config
        ...
        if (!printWindow) {
            alert('Please allow popups to print the journal');
