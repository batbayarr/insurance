/**
 * Account List Print System
 * 
 * Handles landscape A4 printing for account list tables
 * 
 * Usage:
 * window.printAccountListTable({
 *     title: 'ДАНСНЫ ЖАГСААЛТ',
 *     allData: accountData,
 *     columnHeaders: ['Дансны код', 'Дансны нэр', 'Дансны төрөл', 'Валют', 'Төлөв']
 * })
 */
...
(function() {
    ...
    /**
     * Map data field names to column headers for account list
     */
    ...
    function generateTableRows(data, columnHeaders, fieldMapping) {
        ...
        data.forEach((item, index) => {
            ...
            columnHeaders.forEach(header => {
                ...
                // Left-align all columns for account list
                html += `<td>${escapeTemplateLiteral(value)}</td>`;
            });
...
<body>
    ...
    <table id="print-table">
        <thead>
            <tr>
                ${config.columnHeaders.map(header => `<th>${escapeTemplateLiteral(header)}</th>`).join('')}
            </tr>
        </thead>
        <tbody>
            ${tableRows}
    ...
    <script>
        ...
        function exportToExcel() {
            ...
            alert('Account list exported to Excel successfully! The file should open automatically if Excel is your default application for CSV files.');
        }
     ...
     * Main print function
     */
    window.printAccountListTable = function(config) {
        // Validate config
        ...
        // Default values
        config.title = config.title || 'ДАНСНЫ ЖАГСААЛТ';
        ...
        if (!printWindow) {
            alert('Please allow popups to print the account list');
